...
config = {'user_modeling': 'None',
                ...
                'rating_dim':1,
                'num_negative': 4,
                'layers': [64,128,256,128,64],  # layers[0] is the concat of latent user vector & latent item vector
    }
...
class Mlp(torch.nn.Module):
    def __init__(self, config, hidden_size):
        ...
        if config['interaction_modeling'] == 'ncf':
            self.input_size += config['rating_dim']
...
def cal_all_pair():
    ...
    train_dataloader = data.DataLoader(train_dataset, batch_size=1, shuffle=True, collate_fn=my_collate, num_workers=1)
    valid_dataloader = data.DataLoader(valid_dataset, batch_size=1, collate_fn=my_collate, num_workers=1)
