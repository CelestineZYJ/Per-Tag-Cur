...
class Lstm(torch.nn.Module):
    ...
    def forward(self, user_feature, hashtag_feature):
        lstm_output, lstm_hidden = self.lstm(torch.FloatTensor(user_feature[0]).view(1, 1, -1))
        for i in user_feature:
            lstm_output, lstm_hidden = self.lstm(torch.FloatTensor(i.view(1, 1, -1)), lstm_hidden)
        user_modeling = lstm_output
        hashtag_modeling = torch.mean(torch.FloatTensor(hashtag_feature), 0)
